...
def main():
    ...
    for key, value in (data['Data']).items():
        ...
        for time_mode in time_modes:
            ...
            if value['RelevantDir'] == 'anon':
                plotloglog(varying=key, 
                relevant_path=relevant_path,
                relevant_dirname=value['RelevantDir'], 
                relevant_column=value['RelevantColumn'],
                legend_mode=value['LegendMode'],
...
def plotloglog(varying, 
    ...
    if legend_mode == 1:
        ...
        for df in dfs:
            ...
            plt.loglog(mean_values.index, mean_values.iloc[:, 4], linewidth=2)
            # plt.loglog(mean_values.index, mean_values.iloc[:, 4], linewidth=2, marker='o')
    ...
    filename = output_path + 'log-log-varying-' + varying + '-' + time_unit + '.pdf'
